...
export default function PageTransitionProvider({ children }) {
  ...
  return (
    <PageTransitionContext.Provider value={{ setStatus }}>
      ...
      {status !== 'idle' && (
        <div
          className={`fixed inset-0 w-screen h-screen bg-[#1e2a35] z-[99999] flex flex-col items-center justify-center pointer-events-none p-6 ${status === 'covering' ? 'wipe-enter' : 'wipe-exit'
            }`}
        >
          {/* Animated/Pulsing Heart Logo */}
          <div className="flex flex-col items-center max-w-md text-center transition-all duration-300">
            <div className="w-16 h-16 rounded-full bg-[#0f7c85]/20 flex items-center justify-center text-[#0f7c85] mb-6 animate-pulse shadow-sm">
            ...
            {/* Health Tip */}
            <span className="text-xs font-bold tracking-[2.5px] uppercase text-[#0f7c85] mb-2 block">HEALTH TIP</span>
            <p className="font-heading font-bold text-lg md:text-xl leading-snug text-white max-w-sm mb-6">
              &ldquo;{currentTip}&rdquo;
            </p>

            {/* Loading Indicator */}
            <div className="flex items-center gap-2 text-white/60 text-xs font-medium tracking-wide">
              <svg className="animate-spin h-4 w-4 text-[#0f7c85]" fill="none" viewBox="0 0 24 24">
